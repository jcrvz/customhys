...
class Population():
    ...
    def random_sample(pop):
        ...
        pop.positions = np.random.uniform(-1, 1, (pop.num_agents,
                                                   pop.num_dimensions))
    ...
    def ga_crossover(self, pairing="cost", crossover="single",
        ...
        num_couples = self.num_agents - num_mates

        # Get the mating pool using the natural selection
        mating_pool_indices = np.argsort(self.fitness)[:num_mates]
        ...
        if pairing == "cost":
            ...
            normalised_cost = self.fitness[mating_pool_indices] - np.min(
                self.fitness[np.setdiff1d(
                    np.arange(self.num_agents), mating_pool_indices)])
        ...
        # No pairing procedure recognised
            ...
            self.__PopulationError("Invalid pairing method")
        ...
        if crossover == "single":
            ...
            crossover_mask = np.tile(
                np.arange(self.num_dimensions), (parent_indices.shape[1], 1)
                ) <= single_points
        ...
        # OPTIMIZE It can be extended to multiple points
            ...
            points = [np.tile(raw_points[:, x], (
                self.num_dimensions, 1)).transpose() for x in
        ...
        self.positions[offspring_indices, :] = offsprings
