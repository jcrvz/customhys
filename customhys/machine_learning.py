...
def obtain_sample_weight(sample_fitness, fitness_to_weight='rank'):
    ...
    if fitness_to_weight == 'linear_reciprocal':
        # f: [a, b] -> (0, 1]
        def weight_conversion(fitness): return a / fitness
    elif fitness_to_weight == 'linear_reciprocal_translated':
        # f: [a, b] -> [1, b-a+1]
        def weight_conversion(fitness): return a * b / fitness - a + 1
    elif fitness_to_weight == 'linear_percentage':
        # f: [a, b] -> [1, 100]
        def weight_conversion(fitness): return 100 * \
            (b - fitness) / (b - a) + 1
        ...
        # f: [a, b] -> [a, b]
        def weight_conversion(fitness): return a + b - fitness
...
class DatasetSequences():
    def __init__(self, sequences, fitnesses, num_operators=None, fitness_to_weight=None):
        ...
        if fitness_to_weight is not None:
            self._sample_weight =\
                tf.constant(obtain_sample_weight(
                    sample_fitness, fitness_to_weight))
        else:
...
def retrieve_model_info(params):
    ...
    essential_attributes = ['file_label',
                            'model_architecture', 'encoder', 'num_steps']
    if not all(attribute in params for attribute in essential_attributes):
        left_attributes = [
            attribute for attribute in essential_attributes if attribute not in params]
        raise Exception(
            f'The following attributes left while retrieving the model info: {left_attributes}')
...
class Encoder():
    def __init__(self, params):
        ...
        if self._architecture_name in ['transformer', 'transformer_orig', 'LSTM_Ragged']:
            ...
            self.__identity_encoder = compose(
                self.__fix_sequence_length, self.__clean_sequence)
...
class ModelPredictorKeras():
    ...
    def __init__(self, params):
        # Get encoder
        params['memory_length'] = params.get(
            'memory_length', params['num_steps'])
    ...
    def __create_keras_model(self):
        ...
        if architecture_name in ['MLP']:
            ...
            self._model.add(tf.keras.layers.Embedding(
                self._num_operators + 1, first_layer_size))
        elif architecture_name in ['LSTM']:
            # LSTM input
            self._model.add(tf.keras.Input(shape=(input_size, 1)))

        # Hidden layers
        num_lstm_layers = sum('LSTM' == layer_type for _,
                              _, layer_type in hidden_layers)
        ...
        self._model.add(tf.keras.layers.Dense(
            self._num_operators, activation='softmax'))
