...
from rpy2.robjects import ListVector, NA_Complex, NA_Character, NA_Integer, NULL
...
packnames = ['flacco', 'dplyr', 'lhs', 'expm', 'mlbench', 'numDeriv', 'shape', 'shiny', 'testthat', 'RANN',
             'mda', 'e1071']
...
def create_feature_object(x, y, minimize=True, lower=0, upper=1, blocks=None, fun=None):
    ...
    if isinstance(lower, (list, np.ndarray)):
        lower = FloatVector(lower)
    if isinstance(upper, (list, np.ndarray)):
        upper = FloatVector(upper)
