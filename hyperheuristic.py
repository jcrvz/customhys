...
class Hyperheuristic:
    ...
    def _get_sample_sequences(self, kw_sequences_params):
        ...
        if kw_sequences_params['retrieve_sequences']:
            ...
            if kw_sequences_params['filters'].get('include_dimensions', True):
                filters['dimensions'] = f'{self.problem["dimensions"]}D'
            
            if kw_sequences_params['filters'].get('features', None) is None:
                # Consider sequences generated only by the current problem
                problem_names = [self.problem['func_name']]
            else:
                problem_names = []
            
            seqfitness_retrieved, seqrep_retrieved = _get_stored_sample_sequences(features=kw_sequences_params['filters'].get('features', None),
...
def _get_stored_sample_sequences(features=None, additional_problems=[], filters=dict(), folder_name='./data_files/sequences/'):
    ...
    def is_valid_file(file_name):
        ...
        if 'dimensions' in filters and filters['dimensions'] != file_name_splitted[1]:
            ...
            return False
        if 'collection' in filters and filters['collection'] not in file_name_splitted[2]:
