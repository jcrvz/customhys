...
class Hyperheuristic:
    ...
    def __get_neural_network_model(self, kw_model_params):
        ...
        # Prepare feature list for model label
        features_list = kw_model_params['sample_sequences_params']['features']
        feature_neural_network = False
        if features_list is None:
            features_list = []
        elif not kw_model_params['sample_sequences_params']['generate_sequences']:
            feature_neural_network = True
        features_list.sort()
        
        # File names
        model_directory = './data_files/ml_models/'
        if not feature_neural_network:
            model_label = self.file_label + '-'.join(features_list)
        else:
            if len(features_list) == 0:
                features_list = ['All']
            model_label = '-'.join([self.heuristic_space_label, 'features'] + features_list)
    ...
    def _get_sample_sequences(self, kw_sequences_params):
        ...
        :param dict kw_sequences_params: Store the following values,
            ...
            :param str features: Specify which kind of sequences would be retrived
        ...
        if kw_sequences_params['retrieve_sequences']:
            if kw_sequences_params['features'] is None:
                # Retrieve sequences generated by solving this problem
                problem_name = self.file_label.split('-')[0]
                seqfitness_retrieved, seqrep_retrieved = _get_stored_sample_sequences(additional_problems=[problem_name], collection=self.heuristic_space_label)
            else:
                # Retrieve sequences generated by solving problems with such features
                seqfitness_retrieved, seqrep_retrieved = _get_stored_sample_sequences(features=kw_sequences_params['features'], collection=self.heuristic_space_label)
...
def _get_stored_sample_sequences(features=None, additional_problems=[], collection=None, folder_name = './data_files/sequences/'):
    """
    ...
    def is_valid_file(file_name):
        ...
        if file_name_splitted[0] not in valid_problems:
            ...
            return False
        if collection is not None and collection not in file_name_splitted:
